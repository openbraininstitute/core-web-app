      ...
      if (result) {
        ...
        delay(() => {
          onNextStep({
            step: 'project',
            meta: {
              virtualLabId: result.data?.virtual_lab.id!,
              virtualLabName: result.data?.virtual_lab.name!,
            },
          });
          onTransition(false);
        }, 1000);
    ...
    >
      <Card className="mr-4 ml-4 flex w-full max-w-lg min-w-lg flex-col bg-transparent shadow-none backdrop-blur-sm">
        <CardContent>
          {creationResult && (
            <div
              className={cn(
                ...
                { 'border-green-200 bg-green-50': creationResult.status === 'created' },
                { 'border-red-200 bg-red-50': creationResult.status === 'failed' }
              )}
            >
          ...
          {fields?.reference_email !== initialEmail && fields?.email_status !== 'verified' && (
