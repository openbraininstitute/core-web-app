...
export default function DetailView({ type }: Props) {
  ...
  const content = match<EntityCoreTypeConfig<any>>(entity)
    .with(
      ...
      () => (
        <Summary dataType={ExtendedEntitiesType.ReconstructionMorphology}>
          {(detail) => <MorphologyDetailView detail={detail as IReconstructionMorphology} />}
        </Summary>
      )
    )
    .with({ extendedType: ExtendedEntitiesType.ElectricalCellRecording }, () => (
      <Summary dataType={ExtendedEntitiesType.ElectricalCellRecording}>
