...
/**
 * What a simulation's spikes can be replayed over in 3D, or `null`.
 *
 ...
 * one recorded cell is that model. The three viewers behind this draw a cell
 * very differently — a whole neurite tree, a point — but all take a brightness
 * per cell, so which one runs is a rendering detail rather than a limit on what
 * can be replayed.
 */
export function replaySubject(
  ...
  if (model.type === EntityTypeDict.Circuit) return { circuit: model as ICircuit };
  if (model.type === EntityTypeDict.Memodel) return { memodel: model as TSceneMemodel };
