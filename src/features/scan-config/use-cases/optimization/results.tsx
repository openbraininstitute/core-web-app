...
import { InOutFiles } from '@/features/scan-config/use-cases/optimization/in-out-files';
...
type Props = {
  ...
  /** obi-one + entitycore task types for this optimization workflow (from its definition) */
...
export function OptimizationTab({
  ...
  const { mutateAsync: runOptimization, isPending: runOptimizationPending } = useTaskLaunchMutation(
    {
      context,
  ...
  const { configGenerationLoading, configsResponse, configsLoading } =
    useTaskRunner<TTaskConfigMeta>({
      ...
      pauseExecutionPolling: runOptimizationPending,
  ...
  const onRun = async (configIdsToRun: string[]) => {
    await runOptimization(configIdsToRun);
    act.onLaunched();
  };
        ...
        left={
          <div className="flex h-full w-full flex-col gap-4 overflow-y-hidden">
            <TaskConfigSelectionList
              ...
              loading={loading}
              selectionDisabled={runOptimizationPending}
              fallbackColor="#722ED1"
              context={context}
              executionActivityType={taskTypeBindings.execution}
              pauseStatusPolling={runOptimizationPending}
            ...
            />
            <TaskLaunchButton
              label="Launch optimizations"
              countLabel={launchBtnLabelPrefix}
              pending={runOptimizationPending}
              disabled={runOptimizationPending || resolvedSelectedConfigIds.length === 0}
              onClick={openModal}
