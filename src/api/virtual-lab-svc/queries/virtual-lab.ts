 ...
 * @returns {Promise<VlmResponse<VirtualLabResponse>>} - api response with the created virtual lab.
 * @throws {Error} - Throws an error if the request fails or the response is invalid.
 */
export async function createVirtualLab({
  ...lab
}: VirtualLabPayload): Promise<VlmResponse<VirtualLabResponse>> {
  const session = await getSession();
  try {
    ...
    const result: VlmResponse<VirtualLabResponse> = await response.json();
    return result;
  } catch (error) {
    // TODO: capture exception with sentry
