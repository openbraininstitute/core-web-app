...
import { EASE_OUT, SPRING_GLIDE, SPRING_PRESS } from './ease';
import { capturePointer, TOUCH_GESTURE_CLASS } from './touch';
...
export type MorphingTabsItem = {
  ...
  content: ReactNode;
  disabled?: boolean;
  /** Shown on hover. Sits outside the button, so it also works while disabled. */
  tooltip?: ReactNode;
  /** Trailing control inside the tab. Rendered only while the tab is active. */
  action?: ReactNode;
};
...
export interface MorphingTabsProps {
  ...
  /** Keeps every panel mounted and hides the inactive ones, instead of swapping them. */
  keepMounted?: boolean;
...
/** Half the hairline, so the stroke sits fully inside the clipped root. */
const SURFACE_EDGE = 0.5;
...
function liquidTabPath(
  ...
  const panelBottom = Math.max(RAIL_HEIGHT + PANEL_RADIUS * 2, surfaceHeight - SURFACE_EDGE);
  const left = Math.max(panelLeft, Math.min(panelRight - tabWidth, tabLeft));
  const right = left + tabWidth;
  const top = Math.max(SURFACE_EDGE, RAIL_HEIGHT - TAB_HEIGHT);
  ...
  return [
    ...
    `V${panelBottom - PANEL_RADIUS}`,
    `Q${panelRight} ${panelBottom} ${panelRight - PANEL_RADIUS} ${panelBottom}`,
    `H${panelLeft + PANEL_RADIUS}`,
    `Q${panelLeft} ${panelBottom} ${panelLeft} ${panelBottom - PANEL_RADIUS}`,
    'Z',
  ].join(' ');
}
      ...
      {active && surfaceHost && surfaceWidth > SURFACE_INSET * 2
        ? createPortal(
            <svg
              ...
              className={cn(
                // the surface is the panel: white fill, hairline edge, and it
                // stays under the content (z-0) now that it spans the panel
                'pointer-events-none absolute inset-0 z-0 h-full w-full text-white',
                surfaceClassName
              )}
            >
              <LiquidSurfacePath
...
function LiquidSurfacePath({
  ...
  const path = useTransform(left, (value) =>
    liquidTabPath(value, surfaceWidth, surfaceHeight, tabWidth)
  );
  return (
    <motion.path
      d={path}
...
export function MorphingTabs({
  ...
  const startDrag = useCallback(
    (id: string, event: ReactPointerEvent<HTMLDivElement>) => {
      if (!reorderable || event.button !== 0 || itemMap.get(id)?.disabled || dragRef.current) {
        return;
      }
    ...
    [dragLeft, itemMap, reorderable, slotGap, tabWidth]
  );
  ...
  const moveBy = useCallback(
    ...
    [commitOrder, itemMap, reorderable]
  );
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        'relative isolate flex min-h-0 min-w-0 flex-col overflow-hidden bg-transparent text-[#181818]',
        classNames?.root,
        className
      )}
    >
            ...
            return (
              <SpringTab
                ...
                className={cn(
                  ...
                  !item.disabled && !reorderable && 'cursor-pointer',
                  !item.disabled && reorderable && (isDragging ? 'cursor-grabbing' : 'cursor-grab')
                )}
              ...
              >
                <TabTooltip
                  ...
                  arrowClassName={classNames?.tooltipArrow}
                >
                  <div
                    style={{
                      width: tabWidth,
                  ...
                  >
                    {!isActive ? (
                      <span
                        ...
                        className={cn(
                          ...
                          isDragging
                            ? 'bg-[oklch(0.929_0.013_255.508)]'
                            : 'bg-[oklch(0.984_0.003_247.858)] group-hover:bg-[oklch(0.929_0.013_255.508)]'
                        )}
                      />
                    ) : null}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        isActive && item.action && 'pr-12',
                        isActive ? 'text-primary-8' : 'pb-2 text-neutral-4 hover:text-[#181818]',
                        item.disabled && 'cursor-not-allowed opacity-60 hover:text-neutral-4',
                        classNames?.tab
                      )}
                    >
                      {/* keyboard focus ring; open at the bottom while active so it
                          does not cut across the tab's join with the panel */}
                      <span
                        aria-hidden
                        className={cn(
                          ...
                          isActive
                            ...
                            : 'bottom-2 rounded-[1rem] border-2 border-black/20'
                        )}
                      />
                      {item.icon ? (
                        <span
                          ...
                          className={cn(
                            ...
                            isActive
                              ...
                              : 'bg-[oklch(0.968_0.007_247.896)] text-neutral-4',
                            classNames?.icon
                          )}
                        >
                          {item.icon}
                      ...
                      <span
                        className={cn(
                          'min-w-0 truncate whitespace-nowrap text-lg font-medium leading-[normal] tracking-[-0.01em]',
                          classNames?.label
                        )}
                      >
                        {item.label}
                    ...
                    {isActive && item.action ? (
                      <div
                        className={cn(
                          'absolute right-2 top-1/2 z-20 flex -translate-y-1/2 items-center',
                          classNames?.action
                        )}
                        // the tab owns the drag; the control inside it does not
      ...
      <div
        ...
        className={cn(
          'relative z-20 mx-1 flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1.75rem] text-[#181818]',
          classNames?.content
        )}
      >
        {keepMounted ? (
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {activeItem ? (
              <motion.div
                ...
                className={cn('flex min-h-0 flex-1 flex-col', classNames?.panel)}
              >
                {activeItem.content}
              </motion.div>
            ) : null}
