...
export function Menu({ sessionId, simulationType, modelId, memodelId }: Props) {
  ...
  // Monitor simulation status for low funds errors
  useEffect(() => {
    console.log('Does this run?');
    if (simulationStatus?.status === SimulationStatus.ERROR && simulationStatus?.description) {
      const isLowFundsError =
        simulationStatus.description === messages.LowFundsError ||
        simulationStatus.description === messages.LowFundsErrorNonAdmin;

      // Debug
      console.log('Simulation status error detected:', {
      ...
      if (isLowFundsError) {
        ...
        setShowLowFundsNotification(false);
      }
    } else if (simulationStatus?.status !== SimulationStatus.ERROR) {
      setShowLowFundsNotification(false);
    }
  }, [simulationStatus]);
