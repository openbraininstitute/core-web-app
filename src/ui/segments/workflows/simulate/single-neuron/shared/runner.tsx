...
type InsufficientCreditsCardProps = {
  isAdmin: boolean;
  adminEmail?: string;
  onAddCredits?: () => void;
};
...
>(
  ...
  async (
    ...
    creditsCardProps: InsufficientCreditsCardProps
  ) => {
    ...
    try {
      ...
      if (error) {
        ...
        const notificationKey = `simulation-failed-${sessionId}`;
        notify.error({
          ...
          placement: 'topRight',
          key: notificationKey,
          duration: isLowFundsError ? 0 : 1000,
        });
        return;
      }
