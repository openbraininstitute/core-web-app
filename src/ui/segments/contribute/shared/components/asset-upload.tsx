...
export function AssetUpload({
  ...
  // [DIAG 4] Check CSP headers and whether blob URLs are usable on mount
  useEffect(() => {
    const csp = document
      .querySelector('meta[http-equiv="Content-Security-Policy"]')
      ?.getAttribute('content');
    console.debug('[AssetUpload] CSP meta tag:', csp ?? 'none (may be header-only)');

    try {
      const testBlob = new Blob(['test'], { type: 'text/plain' });
      const url = URL.createObjectURL(testBlob);
      URL.revokeObjectURL(url);
      console.debug('[AssetUpload] createObjectURL: OK');
    } catch (e) {
      console.error('[AssetUpload] createObjectURL BLOCKED:', e);
    }
  }, []);
  ...
  const [
    ...
    async onFilesAdded(addedFiles, setState) {
      // [DIAG 5] Log onFilesAdded entry — if this never appears on Safari remotely,
      // the failure is inside addFiles/validateFile, not here
      console.debug('[AssetUpload] onFilesAdded called', {
