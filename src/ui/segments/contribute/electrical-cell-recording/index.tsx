...
import {
  ...
  License,
} from '@/ui/segments/contribute/electrical-cell-recording/license';
import { Contribution } from '@/ui/segments/contribute/electrical-cell-recording/contribution';
import { ETypeClassification } from '@/ui/segments/contribute/electrical-cell-recording/etype';
import { AssetUpload } from '@/ui/segments/contribute/electrical-cell-recording/asset-upload';
import { usePipeline } from '@/ui/segments/contribute/electrical-cell-recording/use-pipeline';
import { useBrainRegionHierarchy } from '@/features/brain-region-hierarchy/context';
import { Tooltip, TooltipContent, TooltipTrigger } from '@/ui/molecules/tooltip';
import { Subject } from '@/ui/segments/contribute/electrical-cell-recording/subject';
import { AppUInterfaceSection, resolveDataKey } from '@/utils/key-builder';
import { Setup } from '@/ui/segments/contribute/electrical-cell-recording/setup';
import {
  SubmitButton,
  SubmitEntityProgress,
} from '@/ui/segments/contribute/electrical-cell-recording/submit-entity';
import {
...
function UploadElectricalCellRecording({ brainRegionId, sessionId }: Props) {
  const [form] = Form.useForm<TElectricalCellRecordingForm>();
  ...
  const parseContribution = ElectricalCellRecordingSchema.pick({ contribution: true }).safeParse(
    allValues
  );
  const parseEtype = ElectricalCellRecordingSchema.pick({ etype_class_id: true }).safeParse(
    allValues
  );
  const parseSubject = ElectricalCellRecordingSchema.pick({ subject_id: true }).safeParse(
    allValues
  );
  const parseLicense = ElectricalCellRecordingSchema.pick({ license_id: true }).safeParse(
    allValues
  );
  const parseAssets = ElectricalCellRecordingSchema.pick({ assets: true }).safeParse(allValues);
  const parseSetup = ElectricalCellRecordingSchema.pick({ setup: true }).safeParse(allValues);
  ...
  const onSubmit = async (values: TElectricalCellRecordingForm) => {
    setSubmitting(true);
  ...
  return (
    <div className={cn('relative mx-auto h-full w-full px-6 py-2')}>
      <Form
        form={form}
        id="contribute-electrical-cell-recording-modal"
        rootClassName={cn(
      ...
      >
          ...
          {submitting ? (
            ...
            <div className="relative h-full w-full">
              {steps.map((step) => (
                <motion.div
                  key={step.key}
                  id={`electrical-cell-recording-${step.key}`}
        ...
        <div className="flex w-full flex-shrink-0 items-center justify-between gap-2 py-3">
          ...
          <Button
            ...
            className="text-primary-9 border-primary-9 disabled:border-neutral-1 shadow-bnb size-12 active:text-white"
            onClick={onNext}
            disabled={activeStep === STEPS.etype}
          >
            <RightOutlined />
          </Button>
        </div>
...
export function ElectricalCellRecording({ sessionId }: { sessionId: string }) {
  ...
  return (
    <UploadElectricalCellRecording brainRegionId={defaultBrainRegion.id} sessionId={sessionId} />
  );
}
