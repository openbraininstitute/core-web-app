...
export function PreMTypeClassificationSelector<TSchema extends ZodObject<ZodRawShape>>({
  schema,
}: Props<TSchema>) {
  ...
  return (
    <>
      <Form.Item
        name="pre_mtype_id"
        label={renderLabel('Pre mtype classification', 'main', RequiredFieldMarker)}
        rules={[
          {
            required: true,
            validator: createZodFieldValidator(schema, 'pre_mtype_id', form),
          },
        ]}
      >
        <MTypeDropdown />
