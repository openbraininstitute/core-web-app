...
const { getEntity, getCircuit, getSimulationCampaign, getTaskConfig } =
  vi.hoisted(() => ({
    getEntity: vi.fn(),
    getCircuit: vi.fn(),
...
vi.mock(
  "@/api/entitycore/queries/simulation/campaign",
...
vi.mock(
  "@/api/entitycore/queries/task/task-config",
  async (importOriginal) => ({
    ...(await importOriginal<object>()),
    getTaskConfig,
  }),
...
const allFlags = Object.fromEntries(
  flags.map((flag) => [
    flag.key,
    typeof flag.defaultValue === "boolean" ? true : flag.defaultValue,
  ]),
) as FeatureFlags;
...
const cases: TCase[] = [
  ...
  },
  {
    ...
    fixture: {
      ...
      input: { type: EntityTypeDict.Circuit },
      taskConfigType:
        TaskConfigType.ExtracellularRecordingWeightsCalculationCampaign,
    },
    covers: {
      activity: build,
      targetType: ExtendedEntitiesTypeDict.ExtracellularRecordingArrayCampaign,
...
describe("resolveWorkflowConfigureHrefForEntity", () => {
  ...
  it("covers every workflow an entity id can reach", () => {
    const covered = new Set(
      cases.map(({ covers }) => `${covers.activity}/${covers.targetType}`),
    );
    ...
    const uncovered = Object.values(ActivityRegistry).flatMap((activity) =>
      activity.workflows
        ...
        .filter(
          (key) => !covered.has(key) && !UNREACHABLE_FROM_AN_ENTITY_ID.has(key),
        ),
    );
  ...
  it.each([EntityLifecycleStatus.Draft, EntityLifecycleStatus.Disqualified])(
    "reports the block reason for a %s source model",
