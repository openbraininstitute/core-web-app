...
describe('LargeCircuitPreview camera focus', () => {
  beforeEach(() => {
    fixtures.rendered = [];
    fixtures.placement = {
      placed: [
  ...
  it('names the slice of the scene the population on show holds', () => {
    draw({ population: VPM });

    const props = lastRender();
    expect(props.cameraFocus).toEqual({ from: 2, count: 1 });
    // The range indexes somas, not floats, and the viewer reads it against
    // exactly this array.
    const { from, count } = props.cameraFocus ?? { from: 0, count: 0 };
    expect(from + count).toBeLessThanOrEqual((props.positions?.length ?? 0) / 3);
  });

  // The whole point of the range: putting another population on show tells the
  // camera where to go next, and does not send it there.
  it('moves the range on a selection change without moving the scene', () => {
    const { rerender } = draw({ population: CORTEX });
    const first = lastRender();
    expect(first.cameraFocus).toEqual({ from: 0, count: 2 });

    rerender(preview({ population: VPM }));
    const second = lastRender();

    expect(second.cameraFocus).toEqual({ from: 2, count: 1 });
    expect(second.positions).toBe(first.positions);
  });

  // There is nothing on screen to frame, and framing where it would have been
  // would point the camera at empty space.
  it('names nothing when the population on show is hidden', () => {
    draw({ population: CORTEX, hiddenPopulations: ['cortex'] });

    expect(lastRender().cameraFocus).toBeNull();
  });

  it('names nothing when the population on show could not be placed', () => {
    fixtures.placement = {
      placed: [{ population: CORTEX, geometry: placement([0, 0, 0, 1, 1, 1]) }],
      failures: new Map([['vpm', new Error('no x/y/z columns; nothing to place in 3D')]]),
      settled: true,
