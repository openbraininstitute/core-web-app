...
describe('column chooser — unnamed columns', () => {
  const withActions: IGridSchema<Row> = {
    ...SCHEMA,
    columns: [
      ...COLUMNS,
      // an unnamed, permanently-visible action column
      { id: 'actions', header: '', alwaysVisible: true },
    ],
  };

  it('does not offer a blank, permanently-disabled checkbox for it', async () => {
    mount(withActions);
    fireEvent.click(screen.getByRole('button', { name: 'Columns' }));

    const boxes = await screen.findAllByRole('checkbox');
    // Select all + the three named columns; the unnamed one is not listed
    expect(boxes).toHaveLength(4);
    expect(boxes.some((b) => b.getAttribute('value') === 'actions')).toBe(false);
  });

  it('keeps it visible through a bulk deselect all the same', async () => {
    const { controller } = mount(withActions);
    fireEvent.click(screen.getByRole('button', { name: 'Columns' }));
    fireEvent.click(await screen.findByRole('checkbox', { name: 'Select all' }));

    // not listed is not the same as not tracked: `alwaysVisible` still protects it
    expect(controller.store.getSnapshot().hiddenColumns).not.toContain('actions');
  });

  it('still lists an unnamed column the user is allowed to hide', async () => {
    mount({ ...SCHEMA, columns: [...COLUMNS, { id: 'blank', header: '' }] });
    fireEvent.click(screen.getByRole('button', { name: 'Columns' }));

    const boxes = await screen.findAllByRole('checkbox');
    expect(boxes.some((b) => b.getAttribute('value') === 'blank')).toBe(true);
  });
});
