...
export type TBuildSynaptomeCampaignMeta = {
...
async function list({
  ...
  return Task.many<TBuildSynaptomeCampaignMeta>({
    context,
    withFacets,
    ...TaskFlow,
    filters: {
...
async function rows({
  ...
  campaign?: ITaskConfig<TBuildSynaptomeCampaignMeta>;
  id: string;
  context: WorkspaceContext | undefined;
}) {
  return Task.one<TBuildSynaptomeCampaignMeta>({
    campaign,
    id,
    context,
    ...TaskFlow,
...
async function resolve({ id, context }: { id: string; context?: WorkspaceContext | null }) {
  ...
  if (!campaign) {
    throw new Error(`No build synaptome campaign with id ${id} found`);
  }

  const taskRows = await Task.one<TBuildSynaptomeCampaignMeta>({
    id,
...
export type TExtendedBuildSynaptomeCampaignsType = AwaitedType<ReturnType<typeof list>>;

export type TResolvedBuildSynaptomeByCampaign = Awaited<ReturnType<typeof resolve>>;
export type TResolvedBuildSynaptomeByCampaigns = Awaited<ReturnType<typeof list>>;

export const BuildSynaptomeCampaign: EntityCoreTypeConfig<
  ITaskConfig<TBuildSynaptomeCampaignMeta>,
  TResolvedBuildSynaptomeByCampaign,
...
> = {
  ...
  api: {
    ...
    expandRow: async (record, context) =>
      rows({
        campaign: record as ITaskConfig<TBuildSynaptomeCampaignMeta>,
        id: record.id,
        context,
      }),
  },
