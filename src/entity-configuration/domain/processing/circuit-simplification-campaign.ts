...
export type TCircuitSimplificationTaskConfigMeta = {
  scan_parameters?: Record<string, unknown>;
};

const TaskFlow: TTaskFlowTypes = {
  campaignConfigType: TaskConfigType.CircuitSimplificationCampaign,
};
...
async function list({
  ...
  return Task.many<TCircuitSimplificationTaskConfigMeta>({
    context,
    withFacets,
    ...TaskFlow,
    filters: {
...
async function rows({
  ...
  campaign?: ITaskConfig<TCircuitSimplificationTaskConfigMeta>;
  id: string;
  context: WorkspaceContext | undefined;
}) {
  return Task.one<TCircuitSimplificationTaskConfigMeta>({
    campaign,
    id,
    context,
    ...TaskFlow,
...
async function resolve({ id, context }: { id: string; context?: WorkspaceContext | null }) {
  ...
  const campaign = await getTaskConfig<TCircuitSimplificationTaskConfigMeta>({
    id,
    context: resolvedContext,
  });

  if (!campaign) {
    throw new Error(`No circuit simplification campaign with id ${id} found`);
  }

  const taskRows = await Task.one<TCircuitSimplificationTaskConfigMeta>({
    id,
    context: resolvedContext,
    ...TaskFlow,
  });
...
export type TExtendedCircuitSimplificationCampaignsType = AwaitedType<ReturnType<typeof list>>;

export type TResolvedCircuitSimplificationByCampaign = Awaited<ReturnType<typeof resolve>>;
export type TResolvedCircuitSimplificationByCampaigns = Awaited<ReturnType<typeof list>>;

export const CircuitSimplificationCampaign: EntityCoreTypeConfig<
  ITaskConfig<TCircuitSimplificationTaskConfigMeta>,
  TResolvedCircuitSimplificationByCampaign,
  TResolvedCircuitSimplificationByCampaigns
> = {
  group: EntityTypeGroup.Processing,
  title: 'Circuit simplification campaign',
  extendedType: ExtendedEntitiesTypeDict.CircuitSimplificationCampaign,
  type: EntityTypeDict.TaskConfig,
  slug: EntitySlug.CircuitSimplification,
  api: {
    config: {
      allowedFacets: true,
    ...
    expandRow: async (record, context) =>
      rows({
        campaign: record as ITaskConfig<TCircuitSimplificationTaskConfigMeta>,
        id: record.id,
        context,
      }),
  },
