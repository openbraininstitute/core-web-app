...
const TaskFlow: TTaskFlowTypes = {
  campaignConfigType: TaskConfigType.EFeatureExtractionCampaign,
};
...
async function resolve({ id, context }: { id: string; context?: WorkspaceContext | null }) {
  ...
  if (!campaign) {
    throw new Error(`No e-feature extraction campaign with id ${id} found`);
  }
  ...
  // extraction runs on one or more recordings, unlike the single-input circuit extraction
  const recordingIds = (firstConfig?.inputs ?? []).map((input) => input.id);
...
export type TExtendedEFeatureExtractionCampaignsType = AwaitedType<ReturnType<typeof list>>;

export type TResolvedEFeatureExtractionByCampaign = Awaited<ReturnType<typeof resolve>>;
export type TResolvedEFeatureExtractionByCampaigns = Awaited<ReturnType<typeof list>>;

export const EFeatureExtractionCampaign: EntityCoreTypeConfig<
  ITaskConfig<TTaskConfigMeta>,
  TResolvedEFeatureExtractionByCampaign,
  TResolvedEFeatureExtractionByCampaigns
> = {
  ...
  slug: EntitySlug.EFeatureExtraction,
  api: {
    config: {
      allowedFacets: true,
