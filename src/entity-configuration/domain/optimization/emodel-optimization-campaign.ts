...
const TaskFlow: TTaskFlowTypes = {
  campaignConfigType: TaskConfigType.EModelOptimizationCampaign,
};
...
async function resolve({ id, context }: { id: string; context?: WorkspaceContext | null }) {
  ...
  if (!campaign) {
    throw new Error(`No e-model optimization campaign with id ${id} found`);
  }
  ...
  const inputIds = (firstConfig?.inputs ?? []).map((input) => input.id);
...
export type TExtendedEModelOptimizationCampaignsType = AwaitedType<ReturnType<typeof list>>;

export type TResolvedEModelOptimizationByCampaign = Awaited<ReturnType<typeof resolve>>;
export type TResolvedEModelOptimizationByCampaigns = Awaited<ReturnType<typeof list>>;

export const EModelOptimizationCampaign: EntityCoreTypeConfig<
