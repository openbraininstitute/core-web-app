...
import { SingleGraphDataProps } from '@/components/explore-section/Circuit/type';
...
export default function PieCharts({
  ...
  useEffect(() => {
    ...
    const pie = d3.pie<PieDataProps>().value((d) => d.value);

    // Create arc generator for the pie slices
    const arc = d3.arc<d3.PieArcDatum<PieDataProps>>().innerRadius(0).outerRadius(radius);
  ...
  return (
    <div className="relative flex flex-col self-start justify-self-start">
      <div className="relative mb-4 flex flex-col text-primary-9">
        <div className="text-base font-normal">Fig. {index + 1}</div>
