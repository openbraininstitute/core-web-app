...
export function useToggleColumns<T extends { key: string; hidden: boolean }>(initialColumns: T[]) {
...
export function useFilters<T>(data: T[]) {
  ...
  const filter = useCallback(
    (
      ...
      filterFun: (
        value: T[keyof T],
        inputValue: ChangeEvent<HTMLInputElement>['currentTarget']['value']
      ) => boolean
    ) => {
      return (e: ChangeEvent<HTMLInputElement>) => {
        setFilters({
          ...filters,
          [dataIndex]: (d: T[keyof T]) => filterFun(d, e.currentTarget.value),
        });
      };
    },
    []
